...
int auxiliary_sub(s21_decimal value_1, s21_decimal value_2, s21_decimal *result){
    ...
    if(!result || !is_correct_decimal(value_1) || !is_correct_decimal(value_2)){
        ...
        *result = create_zero_decimal();
        ...
        if(sign_1 == 0 && sign_2 == 0){
            ...
            if(s21_is_greater_or_equal(value_1,value_2)){
                status = auxiliary_sub(s21_decimal_abs(value_2),s21_decimal_abs(value_1),&res);
            }else{
                status = auxiliary_sub(s21_decimal_abs(value_1),s21_decimal_abs(value_2),&res);
                s21_negate(res,&res);
            }
        }
