...
s21_decimal mul_by_ten(s21_decimal value) {
    ...
    double_decimal temp_3 = double_decimal_add(
        create_double_decimal_from_decimal(temp_1),
...
int get_bit_big(double_decimal value, int bit) {
    return !!(value.decimal[bit / 128].bits[(bit % 128) / 32] & (1u << ((bit % 128) % 32)));
}
...
void set_bit_big(double_decimal *value, int position, int sum) {
    if (sum) {
        value->decimal[position / 128].bits[(position % 128) / 32] |= (1u << (position % 32));
    } else {
        value->decimal[position / 128].bits[(position % 128) / 32] &= ~(1u << (position % 32));
    }
}
...
int double_decimal_binary_compare(double_decimal value_1, double_decimal value_2) {
    int result_of_compare = s21_decimal_binary_compare(value_1.decimal[1], value_2.decimal[1]);
    if (result_of_compare == 0) {
        result_of_compare = s21_decimal_binary_compare(value_1.decimal[0], value_2.decimal[0]);
    }
...
int zero_or_one_insertion(s21_decimal *value_1, s21_decimal *value_2, s21_decimal *Q) {
    int status = 0;
